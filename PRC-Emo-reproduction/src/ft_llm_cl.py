import os
# Respect CUDA_VISIBLE_DEVICES supplied by the launcher.
...
class SimplifiedTrainer(SFTTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._raw_eval_dataset = kwargs.get('eval_dataset', None)
        self.eval_dataset = self._process_raw_data(self._raw_eval_dataset)
        print("len(eval dataset) = ",  len(self.eval_dataset))

    def _build_eval_records(self, dataset):
        """Keep raw prompts/messages aligned with evaluation outputs."""
        records = []
        if dataset is None:
            return records
        for index, sample in enumerate(dataset):
            ...
            if messages:
                try:
                    ...
                    prompt = ""
                if len(messages) >= 2:
                    utterance = messages[-2].get("content", "")
                if isinstance(messages[-1], dict):
                    gold_label = messages[-1].get("content", "")
            records.append({
                "index": index,
                "utterance": utterance,
    ...
    def get_eval_dataloader(self, eval_dataset=None) -> DataLoader:
        ...
        if eval_dataset is None:
            ...
            raw_eval_dataset = self._raw_eval_dataset
        else:
            raw_eval_dataset = eval_dataset

        # Save raw records before TRL removes the messages column.
        self._active_eval_records = self._build_eval_records(raw_eval_dataset)
    ...
    def evaluation_loop(
        ...
        with torch.no_grad():
            for step, inputs in enumerate(tqdm(dataloader)):
                ...
                start = len(all_eval_records)
                active_records = getattr(self, "_active_eval_records", [])
                batch_records = active_records[start:start + len(str_labels)]
                if len(batch_records) < len(str_labels):
                    batch_records += [
                        {"index": start + i, "utterance": "", "gold_label": "", "prompt": "", "messages": []}
                        for i in range(len(str_labels) - len(batch_records))
                    ]
                all_eval_records.extend(batch_records)
        ...
        failure_samples = []
        for index, (prediction, label, raw_output) in enumerate(zip(all_preds, all_labels, all_raw_decoded)):
            if prediction != label:
                record = all_eval_records[index] if index < len(all_eval_records) else {}
        ...
        with open(
            ...
            json.dump(result_data, result_file, indent=1, ensure_ascii=False)
...
if __name__=='__main__':
    ...
    parser.add_argument('--base_model_id', type=str, default='/home/pc/jcy/models/Qwen2.5-7B-Instruct', help='local Qwen2.5 base model path')
    ...
    attn_impl = os.environ.get("PRC_EMO_ATTN_IMPL", "flash_attention_2")
