...
CAPTION_PROMPT_MODE="${CAPTION_PROMPT_MODE:-separate_fields}"
...
CAPTION_DATA_DIR="${CAPTION_DATA_DIR:-../PROCESSED_DATASET/meld/omni_caption_prompt/window_${HISTORICAL_WINDOW}_${PROMPT_STYLE}_clean_separate}"

RUN_ID="${RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
SEEDS="${SEEDS:-42 66 100}"
...
TRAIN_PORTS="${TRAIN_PORTS:-29700 29701 29702}"
...
if [ "${#SEED_ARRAY[@]}" -lt 3 ]; then
    echo "SEEDS must contain at least three seeds, for example: SEEDS=\"42 66 100\""
    exit 1
fi
...
if [ "${#PORT_ARRAY[@]}" -lt 3 ]; then
    echo "TRAIN_PORTS must contain at least three ports, for example: TRAIN_PORTS=\"29700 29701 29702\""
    exit 1
fi
...
echo "******************************************************************************************"
echo "MELD clean Omni caption 3-seed run"
echo "Run id: ${RUN_ID}"
echo "Seeds: ${SEEDS}"
...
LOG_DIR="${OUTPUT_BASE}/parallel_logs_clean_3seed_${RUN_ID}"
mkdir -p "${LOG_DIR}"
...
launch_train() {
    ...
    local port="$2"
    local seed="$3"
    local run_name="omni_clean_separate_seed${seed}"
    local output_dir="${OUTPUT_BASE}/LR_${LORA_LR}_BS_${BATCH_SIZE}_${run_name}_run_${RUN_ID}"
...
launch_train "${DEVICE_ARRAY[0]}" "${PORT_ARRAY[0]}" "${SEED_ARRAY[0]}"
launch_train "${DEVICE_ARRAY[1]}" "${PORT_ARRAY[1]}" "${SEED_ARRAY[1]}"
launch_train "${DEVICE_ARRAY[2]}" "${PORT_ARRAY[2]}" "${SEED_ARRAY[2]}"
